...
class ViT(nn.Sequential):
    def __init__(self, Nf, num_classes, num_pcs, n_time_steps:int,emb_size=10, depth=1, kc=51, num_heads=5, **kwargs):
        super().__init__(
            ...
            # ResidualAdd(
            #     nn.Sequential(
            #         nn.LayerNorm(192), # changed from n_time_step to 192 for PCA size
            #         channel_attention(n_time_steps, num_classes),
...
class channel_attention(nn.Module):
    def __init__(self, sequence_num, n_classes, inter=30): # TODO: why is sequence_num the same as n_time_steps
        ...
        self.query = nn.Sequential(
            nn.Linear(50,50),
            nn.LayerNorm(50),  # also may introduce improvement to a certain extent
    ...
    def forward(self, x):
        ...
        temp = rearrange(x, 'c s-> s c')
        temp_query = rearrange(self.query(temp), 's c -> c s')
        temp_key = rearrange(self.key(temp), 's c ->c s')
        ...
        channel_atten = torch.einsum('c s, m s -> c m', channel_query, channel_key) / scaling

        channel_atten_score = F.softmax(channel_atten, dim=-1)
        channel_atten_score = self.drop_out(channel_atten_score)

        out = torch.einsum('c s, c m -> c s', x, channel_atten_score)
        '''
        projections after or before multiplying with attention score are almost the same.
        '''
        out = rearrange(out, 'c s -> s c')
        out = self.projection(out)
        out = rearrange(out, 's c -> c s')
        return out
...
class Trans():
    ...
    def trainTest(self, num_folds:int, params, log=''):
        ...
        # mu, sigma = self.preprocessing(X_t, y_t)
        ...
        X_t = np.abs(X_t) # add channel dimension for convolution
        X_v = np.abs(X_v)
...
def main():
    ...
    FILENAME = r'cwt_out.npy'
    result_write = open(os.path.join(OUTDIR, 'cwt_sub_result.csv'), "w") # TODO: EDIT PATH
